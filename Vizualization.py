...
if start_button:
    with col1 :
        ...
        if not chk_all:
            ...
            st.subheader(f'해당 날짜의 목적지까지 평균 혼잡도는 :red[{round(conge2*100,1)}%]입니다(MAE:{round(mae_score2,2)}).')
    ...
    with col2 :
        ...
        st.write(f'▲선택한 기본요금 {radio_select}과 평균이 같은 Dynamic Pricing을 비교한 결과')
        f, e= st.columns([0.5,10])
        with e:
            st.text(f"     "+'▲지하철 수요 탄력성은 -0.283이라는 가정 하에 기본 요금 1250원에서 변하는 수요량과 매출액을 추정')
        base=[1968,2156,238,2423,2556,2678,2450]
        ...
        if not chk_all: 
            ...
            with col4:
                ...
                delta_color="inverse")
            with col5:
                st.metric(label="Mean Charge", value=round(mean_v,-1), delta=round(mean_v-1250,-1),
                delta_color="inverse")
            with col6:
                st.metric(label="per_day_passenger(백명)", value=round(perday_C,1))
            with col7:
            ...
            with col4:
                ...
                delta_color="inverse")
            with col5:
                st.metric(label="Mean Charge", value=round(mean_v2,-1), delta=round(mean_v2-1250,-1),
                delta_color="inverse")
            with col6:
                st.metric(label="per_day_passenger(백명)", value=round(perday_C2,1))
            with col7:
